...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $success = $clientLogoModel->update($id, [
        ...
        if ($success) {
            $_SESSION['flash_message'] = 'Client logo updated successfully.';
            $_SESSION['flash_type'] = 'success';
            header('Location: ' . get_app_base_url() . '/admin/client-logos.php');
            exit;
        } else {
            $errors[] = 'Failed to update client logo. Please try again.';
        }
    }
    
    // Update logo array with submitted values for form display
    $logo = array_merge($logo, [
        'client_name' => $client_name,
        'logo_url' => $logo_url,
...
include_admin_header('Edit Client Logo');
?>
...
<div class="admin-page-header">
    <div class="admin-page-header-content">
        ...
        <h1 class="admin-page-title">Edit Client Logo</h1>
        <p class="admin-page-description">Update client logo details</p>
    </div>
</div>
        ...
        <div class="admin-card-body">
            <div class="form-group">
                ...
                <input type="text" id="client_name" name="client_name" class="form-input" 
                       value="<?php echo htmlspecialchars($logo['client_name']); ?>" required>
            ...
            <div class="form-group">
                ...
                <input type="url" id="logo_url" name="logo_url" class="form-input" 
                       value="<?php echo htmlspecialchars($logo['logo_url']); ?>" required
            ...
            <div class="form-group">
                ...
                <input type="url" id="website_url" name="website_url" class="form-input" 
                       value="<?php echo htmlspecialchars($logo['website_url'] ?? ''); ?>"
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="number" id="display_order" name="display_order" class="form-input" 
                           value="<?php echo htmlspecialchars($logo['display_order']); ?>" min="0">
                ...
                <div class="form-group">
                    ...
                    <select id="status" name="status" class="form-select">
                        <option value="DRAFT" <?php echo $logo['status'] === 'DRAFT' ? 'selected' : ''; ?>>Draft</option>
                        <option value="PUBLISHED" <?php echo $logo['status'] === 'PUBLISHED' ? 'selected' : ''; ?>>Published</option>
                        <option value="ARCHIVED" <?php echo $logo['status'] === 'ARCHIVED' ? 'selected' : ''; ?>>Archived</option>
                    </select>
    ...
    <div class="admin-card">
        ...
        <div class="admin-card-body">
            <div class="logo-preview-container">
                <img id="logo-preview" src="<?php echo htmlspecialchars($logo['logo_url']); ?>" 
                     alt="Logo preview" style="max-height: 60px; max-width: 200px;">
            </div>
        </div>
    </div>
