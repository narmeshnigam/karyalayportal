...
function asset_url($path, $versioned = true) {
    ...
    if ($baseUrl === null) {
        ...
        // Use get_app_base_url() for consistency
        $appBase = get_app_base_url();
        $baseUrl = $appBase . '/assets/';
    }
 ...
 * Works for both local development (XAMPP subdirectory) and production (root domain)
 * 
 * @return string Base URL path (e.g., '/karyalayportal/public' or '/public' or '')
 */
function get_base_url() {
    ...
    if ($baseUrl === null) {
        ...
        // Detect if we're in a subdirectory (like XAMPP htdocs/karyalayportal)
        // by checking if the first part of the path is NOT a known app directory
        ...
        $inAdmin = in_array('admin', $parts);
        $inApp = in_array('app', $parts);
        $inInstall = in_array('install', $parts);
        
        if ($isSubdirectory) {
            // Running in a subdirectory (e.g., /karyalayportal/)
            $baseUrl = '/' . $firstPart . '/public';
        } else {
            // Running at root domain
            // Check APP_URL environment variable for explicit configuration
            $configUrl = getenv('APP_URL');
            if (!empty($configUrl)) {
                $parsedUrl = parse_url($configUrl);
                $configPath = !empty($parsedUrl['path']) && $parsedUrl['path'] !== '/' 
                    ? rtrim($parsedUrl['path'], '/') 
                    : '';
                
                // If APP_URL has a path, use it
                if (!empty($configPath)) {
                    $baseUrl = $configPath;
                    ...
                    // Root domain - public pages are at /public
                    $baseUrl = '/public';
                }
            } else {
                // Default: assume public directory
                $baseUrl = '/public';
            }
        }
    }
 ...
 * Used for app directory, admin directory, and other non-public resources
 * Works for both local development (XAMPP subdirectory) and production (root domain)
 * 
...
function get_app_base_url() {
    ...
    if ($appBaseUrl === null) {
        ...
        // Known app directories that are NOT subdirectory names
        $knownAppDirs = ['public', 'admin', 'app', 'install', 'api'];
        $firstPart = reset($parts) ?: '';
        ...
        if ($isSubdirectory) {
            ...
            if (!empty($configUrl)) {
                ...
                if (str_ends_with($configPath, '/public')) {
                    $appBaseUrl = substr($configPath, 0, -7);
                } else {
                    $appBaseUrl = $configPath;
                }
            } else {
                // Root domain - no prefix needed
                $appBaseUrl = '';
            }
        }
    }
